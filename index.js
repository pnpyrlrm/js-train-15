...
function roundObjectValues(obj) {
  ...
  if (typeof obj == "object" && obj != null) {
    ...
    let roundedEntries = entries.map(function ([key, value]) {
    ...
    let newObj = Object.fromEntries(roundedEntries);
    return newObj;
  } else {
...
function estimateInvestment(principal, interestRate, years) {
  ...
  if (principal == NaN && interestRate == NaN && years == NaN) {
    return null;
  } else {
    let sum = principal * (1 + interestRate) ** years;
    // math.pow не працює належним чином (виводить 1276281562500000), тому дідівський метод
...
function isTotalPriceExceedsMaxPrice(products, maxPrice) {
  ...
  if (Array.isArray(products) && maxPrice != NaN) {
